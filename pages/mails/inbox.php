...
$mails = mysqli_query($conn, "
    SELECT mails_tbl.*, users_tbl.full_name AS sender_name, tasks_tbl.task_name
    FROM mails_tbl
    INNER JOIN users_tbl ON users_tbl.user_id = mails_tbl.sender_id
    INNER JOIN tasks_tbl ON tasks_tbl.task_id = mails_tbl.task_id
    WHERE mails_tbl.receiver_id = '$current_user_id'
...
<head>
    ...
    <title>Task Management | Inbox</title>
        ...
        <section class="section">
            <div class="container-fluid">
                <div class="page-header">
                    <div class="header-icon"><i class="lni lni-inbox"></i></div>
                    <div>
                        <h2 class="header-title">Inbox</h2>
                        <p class="header-subtitle">All task mails received from your team</p>
                    </div>
                </div>

                <div class="mail-card">
                    <div class="mail-tabs">
                        <a class="mail-tab active" href="inbox.php"><i class="lni lni-inbox"></i> Inbox</a>
                        ...
                        <table class="table" data-toggle="table" data-search="true">
                            ...
                            <tbody>
                                <?php if (mysqli_num_rows($mails) == 0) { ?>
                                    <tr><td colspan="6"><div class="empty-state">No received mail yet.</div></td></tr>
                                <?php } ?>
                                <?php while ($mail = mysqli_fetch_array($mails)) { ?>
                                    <tr>
                                        ...
                                        <td class="action-cell">
                                            <div class="action">
                                                <a class="action-btn view" href="view.mail.php?id=<?php echo $mail['mail_id']; ?>&box=inbox"><i class="lni lni-eye"></i></a>
                                                <a class="action-btn delete" href="ctrlData/ctrl.delete.mail.php?id=<?php echo $mail['mail_id']; ?>&box=inbox" onclick="return confirm('Delete this mail?')"><i class="lni lni-trash-can"></i></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php } ?>
